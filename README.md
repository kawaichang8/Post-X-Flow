...
### 宣伝設定（Pro）
- **ユーザー設定のプロモーション**: 設定画面「宣伝設定」で、商品名・リンクURL・誘導文テンプレートを設定可能
- **生成投稿への自動挿入**: Proユーザーで宣伝をONにすると、AI生成投稿の末尾に誘導文を自動追加（`[link]`はURLに置換）
- **デフォルトテンプレート**: 「このアイデアを速く試したい人は→[link]でチェック！」（カスタム可）
- **自然さスコア**: 誘導文追加時は軽微なペナルティを適用し、押し売り感を抑制
